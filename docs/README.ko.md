...
- **다중 시나리오 출력** — 이력서, CV, 성과 리뷰, 승진 사례, 스킬 맵 등
- **반복 품질 루프** — 평가가 갭을 발견, 캡처가 자동 보완

### 신뢰와 이식성

- **반조작** — 모든 출력 항목은 `.meta.md` 소스 매핑으로 검증된 증거에 추적 가능
- **제로 종속성** — 순수 프롬프트 엔지니어링; 빌드 도구 불필요, API 키 불필요, 오프라인 동작

### 적응형과 크로스 플랫폼

- **커리어 단계 인식** — 신입, 미드 커리어, 시니어, 전직 각각에 전략 적응
- **5+ 에이전트 플랫폼** — Claude Code, Codex, Cursor, OpenCode, OpenClaw
